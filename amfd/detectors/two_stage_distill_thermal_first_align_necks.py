...
class TwoStageFGDThermalFstIrRgbDetector(BaseDetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # tensor3 = dict(checkpoint['state_dict'])['neck.lateral_convs.1.conv.weight']
        # tensor4 = dict(self.named_parameters())['teacher_neck.lateral_convs.1.conv.weight'].to('cpu')
        # are_equal_b = torch.allclose(tensor3,tensor4,atol=1e-3)
        # print("neck:",are_equal_b)
        ...
        if self.distill:
            ...
            for item_loc in self.distill_cfg:
                ...
                for item_loss in item_loc.methods:
                    loss_name = item_loss.name
                    if 'ir' in loss_name:
                       distill_losses[loss_name] = self.distill_losses[loss_name](x[int(loss_name[-1])],x_t_ir[int(loss_name[-1])],batch_data_samples)
                    else:
